...
module.exports = function(grid,xCoordinate, yCoordinate) {
	...
	if(northValidForPath) {
		validNeighborsResultsArray.push({y:xCoordinate, x:neighbors.north})
	}
	if(southValidForPath) {
		validNeighborsResultsArray.push({y:xCoordinate,x:neighbors.south})
	}
	if(eastValidForPath) {
		validNeighborsResultsArray.push({y:neighbors.east, x:yCoordinate})
	}
	if(westValidForPath) {
		validNeighborsResultsArray.push({y: neighbors.west, x:yCoordinate})
	}
